...
const name=ref('')
const idPermissions=ref(null)

function FormEditShow(Permission) {
  showFormEdit.value=true
  idPermissions.value=Permission.id
  name.value=Permission.name
}


function addPermissions() {
   store.dispatch('AddPermission',name.value)
...
function EditPermissions() {
  const Permissions= {
    id:idPermissions.value,
    'name':name.value
  }
   store.dispatch('EditPermission',Permissions)
   isLoading.value=true
   showFormEdit.value=false
   name.value=''
   setTimeout(() => {
...
function deletePermissions(id) {
   store.dispatch('deletePermission',id)
   isLoading.value=true
   setTimeout(() => {
    store.dispatch('getPermissions')
     isLoading.value=false
...
<template>
    <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 pr-10 lg:px-8">
      <div class="align-middle rounded-tl-lg rounded-tr-lg inline-block w-full py-4 overflow-hidden bg-white shadow-lg px-12">
        ...
        <div class="flex justify-between">
          <h1 class="text-xl font-bold">Gestion des Permission</h1>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
        ...
        <tbody class="bg-[#f97c92]">
            <tr v-for="Permission in Permissions.data" :key="Permission.id" class="odd:bg-[#f97c92] even:bg-white border-b border-gray-300">
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                   {{Permission.name}}
                </th>
                <td class="px-6 py-4">
                    
                    <button @click="FormEditShow(Permission)" type="button" class="text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 shadow-lg shadow-green-500/50 dark:shadow-lg dark:shadow-green-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Edit</button>
                    <button @click="deletePermissions(Permission.id)" type="button" class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 shadow-lg shadow-red-500/50 dark:shadow-lg dark:shadow-red-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Delete</button>
                </td>
            </tr>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 text-center">
          <div class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:w-full sm:max-w-lg" @click.stop>
            <div class="bg-white p-6">
              <h3 class="text-lg font-medium text-gray-900">Add Permission</h3>
              <div>
                <label>Name</label>
            ...
            <div class="bg-gray-50 px-4 py-3 flex flex-row-reverse">
              <button @click="addPermissions" class="inline-flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-teal-600 shadow-sm hover:bg-teal-700">Add</button>
      ...
      <div v-if="showFormEdit" class="w-full h-screen bg-black/60 z-50 fixed top-0" >
        <div class="flex items-center justify-center min-h-screen px-4 text-center">
          <div class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:w-full sm:max-w-lg" @click.stop>
            ...
            <div class="bg-gray-50 px-4 py-3 flex flex-row-reverse">
              <button @click="EditPermissions" class="inline-flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-teal-600 shadow-sm hover:bg-teal-700">Add</button>
